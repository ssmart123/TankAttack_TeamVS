...
public class GameMgr : MonoBehaviourPunCallbacks
{
    ...
    private int m_Team2Win = 0;         //레드팀 승리 카운트

    [Header("--- WinCount ---")]
    public GameObject WinCountRoot;
    public Text m_BlueTeamWin;
    public Text m_RedTeamWin;
    ...
    private float m_BackLobby = 4;
    //--------------- 팀대전 관련 변수들...


    [Header("---MiniMap---")]
    public RawImage MiniMap;
    public Text MiniMapCountText;
    public float MiniMapShowTime = 30;
    float MiniMapTimeCount = 0;
    ...
    {
        //--------------- 팀대전 관련 변수 초기화
        m_Team1Pos[0] = new Vector3(88.4f, 10.0f, 77.9f);
        m_Team1Pos[1] = new Vector3(61.1f, 10.0f, 88.6f);
        m_Team1Pos[2] = new Vector3(34.6f, 10.0f, 98.7f);
        m_Team1Pos[3] = new Vector3(7.7f, 10.0f, 108.9f);

        m_Team2Pos[0] = new Vector3(-19.3f, 10.0f, -134.1f);
        m_Team2Pos[1] = new Vector3(-43.1f, 10.0f, -125.6f);
        m_Team2Pos[2] = new Vector3(-66.7f, 10.0f, -117.3f);
        m_Team2Pos[3] = new Vector3(-91.4f, 10.0f, -108.6f);
        ...
        m_SoundOnOff = System.Convert.ToBoolean(PlayerPrefs.GetInt("SoundOnOff"));
    ...
    {
        float posX = Random.Range(-100.0f, 100.0f);
        float posY = Random.Range(-100.0f, 100.0f);
        PhotonNetwork.Instantiate("Tank",new Vector3(posX, 10.0f, posY), Quaternion.identity, 0);
    }
    ...
    void EnterChat()
    {
        string msg = "\n<color=#ffffff>[" + PhotonNetwork.LocalPlayer.NickName + "] " + textChat.text + "</color>";
        pv.RPC("LogMsg", RpcTarget.AllBuffered, msg);
    ...
    private void SoundPlay()
    {
        if (MainBGM != null)
        {
            ...
            MainBGM.mute = !m_SoundOnOff;
            MainBGM.volume = m_SoundVolume;
        }
    }
            ...
            foreach (Player _player in players)
            {
                ...
                if (_player.CustomProperties.ContainsKey("curHp") == true) //모든 캐릭터의 에너지바 동기화
                {
                    ...
                    if (a_CurHP <= 0)
                    {
                        ...
                        {
                            GUILayout.Label("<color=Red><size=25>" +
                                "[" + _player.ActorNumber + "]" + _player.NickName + " "
                                + currKillCount + " kill" + "</size></color>"
                                + "<color=Red><size=25>" + " <Die>" + "</size></color>");
                ...
                else
                {
                    ...
                    GUILayout.Label("<color=Red><size=25>" + "[" + _player.ActorNumber + "]"
                        + _player.NickName + " " + currKillCount + " kill" + "</size></color>");
                }
            }
        }
